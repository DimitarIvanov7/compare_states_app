...
const  populations = [
   ...
   11689100, 3956971,  4217737, 12801989,  3193694,
...
const createJSON = (googleTrendsData, populations, searchResults, keyword) => {

}

function createData (googleTrendsData, populations, searchResults, keyword) {
    const objectsModel = googleTrendsData.map((TrendsData, index) => {
        return {
          id: index,
          trendsData: TrendsData,
          competition: populations[index] / searchResults[index]
        }
    });

    console.log(objectsModel);

    const objectsStr = JSON.stringify(objectsModel);

    fs.writeFile(`./jsonData/${keyword}.json`, objectsStr, function(err, result) {
        if(err) console.log('error', err);
    });
}

createData (googleTrendsData, populations, searchResults, "locksmith")
